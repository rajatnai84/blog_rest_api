...
from blogs.serializers import BlogSerializer, CategorySerializer, CommentSerializer
from notification.email_templates import (
    comment_add_to_author_notification,
    comment_delete_by_author_notification,
)
...
class BlogListCreateView(ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        return Blog.objects.prefetch_related("tags", "category").filter(
...
class BlogRetrieveUpdateDestroyView(RetrieveUpdateDestroyAPIView):
    ...
    def retrieve(self, request, *args, **kwargs):
        ...
        response_data["paginated_comments"] = paginator.get_paginated_response(
...
class BlogChangeStatusView(APIView):
    ...
    def post(self, request, pk, status_name):
        try:
            ...
            return Response(
                {"detail": "Blog not founds"}, status=status.HTTP_404_NOT_FOUND
            )
        ...
        if status_name not in [choice[0] for choice in Blog.StatusChoices.choices]:
            return Response(
                {"detail": "Invalid status name."}, status=status.HTTP_400_BAD_REQUEST
            )
...
class CommentCreateView(ListCreateAPIView):
    ...
    def perform_create(self, serializer):
        ...
        subject, message = comment_add_to_author_notification(comment.blog, comment)
...
class CommentRetrieveUpdateDestroyView(RetrieveUpdateDestroyAPIView):
    ...
    def destroy(self, request, *args, **kwargs):
        comment = self.get_object()
        if request.user not in (comment.author, comment.blog.author):
            return Response(
                {"detail": "You are not authorized for this action."},
                status=status.HTTP_401_UNAUTHORIZED,
            )
        if request.user == comment.blog.author:
            subject, message = comment_delete_by_author_notification(
